...
class TradingBot:
    ...
    def _handle_event4(self, request_id, account_name, symbol, price, close_position_quantity, repo_quantity, short_position_quantity, repo_details):
        ...
        try:
            ...
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 2 START TIME: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}")
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 2 API PARAMS: symbol={symbol}, quantity={repo_quantity}, interest_rate={repo_details.get('interest_rate', 10.0)}")
            ...
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 2 API RESPONSE: {json.dumps(repo_response, default=str, indent=2) if repo_response else 'None'}")
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 2 END TIME: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}")
            ...
            if repo_response:
                ...
                self.logger.error(f"[{request_id}][{account_name}] EVENT 4 - STEP 2: Open repo failed")
                return responses, "Open repo failed"
        except Exception as e:
            self.logger.error(f"[{request_id}][{account_name}] EVENT 4 - STEP 2: Error: {str(e)}")
            return responses, f"Open repo error: {str(e)}"
        ...
        try:
            ...
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 3 START TIME: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}")
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 3 API PARAMS: symbol={symbol}, side=ASK, price={price}, quantity={short_position_quantity}, tif={tif}")
            ...
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 3 API RESPONSE: {json.dumps(second_sell_response, default=str, indent=2) if second_sell_response else 'None'}")
            self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 3 END TIME: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}")
            
            if second_sell_response:
                responses.append({"step": "second_sell", "response": second_sell_response})
                self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - STEP 3: Second sell succeeded")
            else:
                self.logger.error(f"[{request_id}][{account_name}] EVENT 4 - STEP 3: Second sell failed")
        except Exception as e:
            self.logger.error(f"[{request_id}][{account_name}] EVENT 4 - STEP 3: Error: {str(e)}")
        
        # Add final completion timestamp
        end_time = time.time()
        self.logger.info(f"[{request_id}][{account_name}] EVENT 4 TOTAL EXECUTION TIME: {end_time - start_time:.3f} seconds")
        self.logger.info(f"[{request_id}][{account_name}] EVENT 4 END TIME: {datetime.datetime.fromtimestamp(end_time).strftime('%Y-%m-%d %H:%M:%S.%f')}")
        self.logger.info(f"[{request_id}][{account_name}] EVENT 4 - DEDICATED HANDLER COMPLETED with {len(responses)} responses")
    ...
    def _process_signal_for_account(self, request_id, account_name, symbol, message, timeframe, data):
        ...
        if trade_sequence.get("direct_execution", False):
            ...
            # Use dedicated Event 4 handler
            responses, error = self._handle_event4(
                request_id,
                account_name,
                symbol,
                price,
                ...
                trade_sequence.get("repo_details", {})
            )
            
            if error:
                self.logger.error(f"[{request_id}][{account_name}] Event 4 execution failed: {error}")
            
            # Update last signal regardless of success/failure
            ...
            return {
                ...
                "orders": responses,
                "message": "Executed Event 4 with direct execution" if error is None else f"Event 4 execution error: {error}",

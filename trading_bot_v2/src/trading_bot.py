    ...
    def determine_trade_type(self, symbol, side, timeframe='1h', account_name='default'):
        ...
        # SELL SIGNAL LOGIC
            ...
            # Event 4: Sell Signal with long position (any size) and no repo
                ...
                return {
                    'steps': ['open_short', 'open_repo', 'open_short'],  # 'open_short' after the repo
                    'position_size': [units_to_close, min_quantity, min_quantity],  # Added quantity for second short
                    'repo_details': repo_details,
                    'sequential': True,
                    'event': 'Event 4',
                    'trade_step_index': 0,     # Index of first trade step (before repo ops)
                    'repo_step_index': 1,      # Index of repo step
                    'post_repo_steps': [2]  # Indices of all trade steps after repo (new property)
                }

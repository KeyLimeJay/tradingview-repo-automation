...
def get_price_precision(symbol):
    """Get price precision for a symbol from environment variables"""
    base_currency = symbol.split('/')[0]
    return int(os.getenv(f'{base_currency}_PRICE_DECIMALS', 2))

def adjust_price(price, side, symbol):
    """
...
def place_order(api_key=None, api_secret=None, symbol=None, side=None, price=None, 
    ...
    try:
        ...
        adjusted_price = adjust_price(price, side, symbol)
...
def get_jwt_token():
    """Get JWT token using the exact approach that works"""
...
def get_repo_details(jwt_token=None, symbol=None, logger=None):
    """
    Get details of open repo contracts for a symbol using POST method
    """
...
def place_repo_order(jwt_token=None, symbol=None, quantity=None, interest_rate=None, 
                     custodian_id=None, side="BID", api_key=None, api_secret=None, logger=None):
    """
    ...
    # NEW: Check if a repo already exists for this symbol using direct API check
    # First get a JWT token (necessary for repo details check)
    jwt_token = get_jwt_token()
    if jwt_token:
        existing_repo = get_repo_details(jwt_token, symbol, log)
        if existing_repo:
            if log:

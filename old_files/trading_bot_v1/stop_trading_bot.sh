...
if [ "$SHOW_HELP" = true ]; then
    ...
    echo ""
    echo "Description:"
    echo "  This script stops the multi-account trading bot and disables monitoring."
...
echo "$(date): Attempting to stop multi-account trading bot..." | tee -a $LOG_FILE $REPORT_FILE
...
# Function to get WebSocket client processes
get_websocket_processes() {
    ps aux | grep "position_websocket.py" | grep -v grep
}

# Function to get all trading-related processes
get_all_trading_processes() {
    ps aux | grep -i "trading" | grep -v grep | grep -v "stop_trading_bot"
}
...
# Look for WebSocket processes
WS_PROCESSES=$(get_websocket_processes)
if [ -n "$WS_PROCESSES" ]; then
    echo "Found WebSocket client processes:" | tee -a $LOG_FILE $REPORT_FILE
    echo "$WS_PROCESSES" | tee -a $LOG_FILE $REPORT_FILE
    
    # Kill WebSocket processes
    echo "Killing WebSocket client processes..." | tee -a $LOG_FILE $REPORT_FILE
    pkill -f "position_websocket.py"
    
    sleep 1
    
    # Check if they're still running
    if [ -n "$(get_websocket_processes)" ]; then
        echo "Forcing termination of WebSocket client processes..." | tee -a $LOG_FILE $REPORT_FILE
        pkill -9 -f "position_websocket.py"
        
        if [ -n "$(get_websocket_processes)" ]; then
            echo "WARNING: Failed to kill some WebSocket client processes" | tee -a $LOG_FILE $REPORT_FILE
            STATUS="PARTIAL"
            DETAILS+=("Failed to kill all WebSocket client processes")
        else
            echo "Successfully terminated all WebSocket client processes" | tee -a $LOG_FILE $REPORT_FILE
            DETAILS+=("Terminated all WebSocket client processes")
        fi
    else
        echo "Successfully terminated all WebSocket client processes" | tee -a $LOG_FILE $REPORT_FILE
        DETAILS+=("Terminated all WebSocket client processes")
    ...
    echo "No WebSocket client processes found" | tee -a $LOG_FILE $REPORT_FILE
    DETAILS+=("No WebSocket client processes were running")
fi

# Check if the strategy monitor is running
MONITOR_PROCESSES=$(ps aux | grep "trading_strategy_monitor.py" | grep -v grep)
if [ -n "$MONITOR_PROCESSES" ]; then
    echo "Found strategy monitor processes:" | tee -a $LOG_FILE $REPORT_FILE
    echo "$MONITOR_PROCESSES" | tee -a $LOG_FILE $REPORT_FILE
    
    # Check if PID file exists
    if [ -f ".monitor_pid" ]; then
        MONITOR_PID=$(cat .monitor_pid)
        echo "Strategy monitor PID file found with PID: $MONITOR_PID" | tee -a $LOG_FILE $REPORT_FILE
        
        if ps -p $MONITOR_PID > /dev/null; then
            echo "Killing strategy monitor process with PID: $MONITOR_PID" | tee -a $LOG_FILE $REPORT_FILE
            kill -15 $MONITOR_PID 2>/dev/null  # Try graceful shutdown first
            sleep 2
            
            if ps -p $MONITOR_PID > /dev/null; then
                echo "Forcing termination of strategy monitor process..." | tee -a $LOG_FILE $REPORT_FILE
                kill -9 $MONITOR_PID 2>/dev/null
            fi
            
            rm .monitor_pid
            echo "Removed monitor PID file" | tee -a $LOG_FILE $REPORT_FILE
        else
            echo "PID $MONITOR_PID not running, cleaning up file" | tee -a $LOG_FILE $REPORT_FILE
            rm .monitor_pid
        fi
    else
        echo "No monitor PID file found, using pkill" | tee -a $LOG_FILE $REPORT_FILE
    fi
    
    # Kill any remaining strategy monitor processes
    pkill -f "trading_strategy_monitor.py"
    sleep 1
    
    if [ -n "$(ps aux | grep 'trading_strategy_monitor.py' | grep -v grep)" ]; then
        echo "Forcing termination of strategy monitor processes..." | tee -a $LOG_FILE $REPORT_FILE
        pkill -9 -f "trading_strategy_monitor.py"
        
        if [ -n "$(ps aux | grep 'trading_strategy_monitor.py' | grep -v grep)" ]; then
            echo "WARNING: Failed to kill some strategy monitor processes" | tee -a $LOG_FILE $REPORT_FILE
            STATUS="PARTIAL"
            DETAILS+=("Failed to kill all strategy monitor processes")
        else
            echo "Successfully terminated all strategy monitor processes" | tee -a $LOG_FILE $REPORT_FILE
            DETAILS+=("Terminated all strategy monitor processes")
        fi
    else
        echo "Successfully terminated all strategy monitor processes" | tee -a $LOG_FILE $REPORT_FILE
        DETAILS+=("Terminated all strategy monitor processes")
    ...
    echo "No strategy monitor processes found" | tee -a $LOG_FILE $REPORT_FILE
    DETAILS+=("No strategy monitor processes were running")
fi
...
echo "-----------------------------------------" | tee -a $LOG_FILE $REPORT_FILE
echo "Multi-account bot shutdown process completed with status: $STATUS" | tee -a $LOG_FILE $REPORT_FILE
echo "Full shutdown report saved to $REPORT_FILE"

# Modify monitor script to respect maintenance mode if needed
if [ -f "../monitor_trading_bot.sh" ] && ! grep -q "maintenance_mode" ../monitor_trading_bot.sh; then
    ...
    echo "Modified parent monitor script to respect maintenance mode" | tee -a $LOG_FILE $REPORT_FILE
